    ...
    <div className="space-y-5 sm:space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        {isAdmin && (
          <button
            ...
            className="flex items-center justify-center space-x-1.5 px-4 py-2.5 bg-blue-600 hover:bg-blue-700 active:scale-95 text-white rounded-xl text-xs font-semibold shadow-sm transition-all cursor-pointer w-full sm:w-auto min-h-[44px]"
          >
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-8 sm:p-12 text-center space-y-3">
          ...
          {isAdmin && (
            <button
              type="button"
              onClick={() => setShowModal(true)}
              className="mt-2 inline-flex items-center justify-center space-x-1.5 px-4 py-2.5 bg-blue-600 text-white rounded-xl text-xs font-semibold hover:bg-blue-700 active:scale-95 min-h-[44px] cursor-pointer"
            >
              <Plus className="h-4 w-4" />
      ...
      {showModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-end sm:items-center justify-center p-0 sm:p-4 z-50 overflow-y-auto">
          <div className="bg-white rounded-t-3xl sm:rounded-2xl max-w-md w-full p-5 sm:p-6 space-y-4 shadow-2xl border border-slate-200 my-0 sm:my-6 max-h-[92dvh] sm:max-h-[88vh] overflow-y-auto animate-in fade-in zoom-in-95 duration-150">
            ...
            <form onSubmit={handleCreateProject} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm sm:text-xs font-medium focus:bg-white focus:ring-2 focus:ring-blue-500 focus:outline-none min-h-[44px]"
                  required
                />
              </div>

              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">Assigned Department Unit</label>
                <select
                  value={team}
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm sm:text-xs font-medium focus:bg-white focus:ring-2 focus:ring-blue-500 focus:outline-none resize-none"
                  required
                />
              </div>
              ...
              <div className="flex items-center justify-end space-x-2 pt-2">
                <button
                  type="button"
                  onClick={() => setShowModal(false)}
                  className="px-4 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-xs font-semibold min-h-[44px] cursor-pointer"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 active:scale-95 text-white rounded-xl text-xs font-semibold shadow-sm min-h-[44px] cursor-pointer"
                >
                  Publish Initiative
                </button>
              </div>
